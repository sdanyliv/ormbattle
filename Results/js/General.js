...
function RenderLINQResults() {
    ...
    for (var i = 0; i < checkedTools.length; i++) {
        ...
        if (max < parseFloat( result)) max = result;
...
for (var i = 0; i < str.length; i++) {
    if (str[i] == chr) { var result = str.substr(0, i) + newchr + str.substr(i + 1, str.length - i); }
    }
    return result;
}
...
function CreateTable() {
    ...
    table.AddRow = function (dataArray, dataPath, rowName, controller, key) {
        ...
        tr.appendChild(name);
        if (dataArray != 0) controller.PreRow(tr);
        else name.colSpan = table.rows[0].cells.length;
        for (var i = 0; i < dataArray.length; i++) {
            if (dataPath != undefined) {
                value = GetPropertyFromPath(dataArray[i], dataPath);
                value = GetResultData(value);
            }
            else value = dataArray[i];
            var td = CreateCell(value, 50);
            if (dataArray != 0) controller.EditCell(td);
            tr.appendChild(td);
        }
        if (dataArray != 0) controller.PostRow(tr);
        this.appendChild(tr);
    }
...
function CreateCell(value, width) {
    ...
    if (value != undefined) result.innerHTML = value;
...
function RowLINQController(back,unit) {
    ...
    result.PostRow = function (row) {
        ...
        for (var i = 0; i < this.Cells.length; i++) {
            ...
            if (value != 'n/a')
                if (this.Back) br.bgColor = GetColorFromResult(this.Max, this.Min, value);
                else br.bgColor = GetColorFromResult(this.Min, this.Max, value);
                else  br.bgColor = '#999999'
        }
...
function GetColorFromResult(min, max, value) {
    if (min != 0) min = Math.log(min);
    if (max != 0) max = Math.log(max);
    ...
    if (kg > kr) color.B = kr * 255 / 2;
    else color.B = kg * 255 / 2;

...
namespace OrmBattle.Tests.Performance
{
    public class EntitySpacesTest : PerformanceTestBase
    {
        ...
        static readonly Func<PerformanceTestDataContext, long, int, IQueryable<Simplests>> _pageQuery = CompiledQuery
            .Compile(
                (PerformanceTestDataContext context, long id, int pgSize) =>
                    context.GetTable<Simplests>().Where(o => o.Id >= id).Take(pgSize));

...
namespace OrmBattle.LLBLGenModel.Northwind.DaoClasses
{
	...
	/// <summary>
	/// General DAO class for the OrderDetails Entity. It will perform database oriented actions for
	/// a entity of type 'OrderDetailsEntity'. This DAO works on an EntityFields object. 
	/// </summary>
	public partial class OrderDetailsDAO : DaoBase
	{
		/// <summary>CTor</summary>
		public OrderDetailsDAO() : base(InheritanceInfoProviderSingleton.GetInstance(), new DynamicQueryEngine(), InheritanceHierarchyType.None, "OrderDetailsEntity", new OrderDetailsEntityFactory())
		{
		}
		...
		internal OrderDetailsDAO(IInheritanceInfoProvider inheritanceInfoProviderToUse, DynamicQueryEngineBase dqeToUse, InheritanceHierarchyType typeOfInheritance, string entityName, IEntityFactory entityFactory) : base(inheritanceInfoProviderToUse, dqeToUse, typeOfInheritance, entityName, entityFactory)
		{
		}


		/// <summary>
		/// Retrieves in the calling OrderDetailsCollection object all OrderDetailsEntity objects which have data in common
		...
		public bool GetMulti(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IPredicateExpression filter, IEntity orderInstance, IEntity productInstance, int pageNumber, int pageSize)
		{
			...
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(LLBLGenModel.Northwind.EntityType.OrderDetailsEntity);
		...
		/// <summary>
		/// Retrieves entities of the type 'OrderDetailsEntity' in a datatable which match the specified filter. 
		...
		/// <summary>
		/// Deletes from the persistent storage all 'OrderDetails' entities which have data in common
		/// with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		/// </summary>
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="orderInstance">OrdersEntity instance to use as a filter for the OrderDetailsEntity objects to delete</param>
		/// <param name="productInstance">ProductsEntity instance to use as a filter for the OrderDetailsEntity objects to delete</param>
		...
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(LLBLGenModel.Northwind.EntityType.OrderDetailsEntity);
		...
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="orderInstance">OrdersEntity instance to use as a filter for the OrderDetailsEntity objects to update</param>
		/// <param name="productInstance">ProductsEntity instance to use as a filter for the OrderDetailsEntity objects to update</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		/// </summary>
		/// <param name="orderInstance">OrdersEntity instance to use as a filter for the OrderDetailsEntity objects</param>
		/// <param name="productInstance">ProductsEntity instance to use as a filter for the OrderDetailsEntity objects</param>
		...
		private IPredicateExpression CreateFilterUsingForeignKeys(IEntity orderInstance, IEntity productInstance, IEntityFields fieldsToReturn)
		{
			...
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)OrderDetailsFieldIndex.OrderId], ComparisonOperator.Equal, ((OrdersEntity)orderInstance).Id));
			}
			if(productInstance != null)
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)OrderDetailsFieldIndex.ProductId], ComparisonOperator.Equal, ((ProductsEntity)productInstance).Id));
			}
			return selectFilter;
		}

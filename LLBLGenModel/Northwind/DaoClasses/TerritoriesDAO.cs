...
namespace OrmBattle.LLBLGenModel.Northwind.DaoClasses
{
	...
	/// <summary>
	/// General DAO class for the Territories Entity. It will perform database oriented actions for
	/// a entity of type 'TerritoriesEntity'. This DAO works on an EntityFields object. 
	/// </summary>
	public partial class TerritoriesDAO : DaoBase
	{
		/// <summary>CTor</summary>
		public TerritoriesDAO() : base(InheritanceInfoProviderSingleton.GetInstance(), new DynamicQueryEngine(), InheritanceHierarchyType.None, "TerritoriesEntity", new TerritoriesEntityFactory())
		{
		}
		...
		internal TerritoriesDAO(IInheritanceInfoProvider inheritanceInfoProviderToUse, DynamicQueryEngineBase dqeToUse, InheritanceHierarchyType typeOfInheritance, string entityName, IEntityFactory entityFactory) : base(inheritanceInfoProviderToUse, dqeToUse, typeOfInheritance, entityName, entityFactory)
		{
		}


		/// <summary>
		/// Retrieves in the calling TerritoriesCollection object all TerritoriesEntity objects which have data in common
		...
		public bool GetMulti(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IPredicateExpression filter, IEntity regionInstance, int pageNumber, int pageSize)
		{
			...
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(LLBLGenModel.Northwind.EntityType.TerritoriesEntity);
		...
		/// <summary>
		/// Retrieves entities of the type 'TerritoriesEntity' in a datatable which match the specified filter. 
		...
		/// <summary>
		/// Deletes from the persistent storage all 'Territories' entities which have data in common
		/// with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		/// </summary>
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="regionInstance">RegionsEntity instance to use as a filter for the TerritoriesEntity objects to delete</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int DeleteMulti(ITransaction containingTransaction, IEntity regionInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(LLBLGenModel.Northwind.EntityType.TerritoriesEntity);
		...
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="regionInstance">RegionsEntity instance to use as a filter for the TerritoriesEntity objects to update</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		/// </summary>
		/// <param name="regionInstance">RegionsEntity instance to use as a filter for the TerritoriesEntity objects</param>
		...
		private IPredicateExpression CreateFilterUsingForeignKeys(IEntity regionInstance, IEntityFields fieldsToReturn)
		{
			...
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)TerritoriesFieldIndex.RegionId], ComparisonOperator.Equal, ((RegionsEntity)regionInstance).Id));
			}
			return selectFilter;
		}
